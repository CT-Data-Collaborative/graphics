$(document).ready(function(){
    ...
    d3.csv("static/data/eligibility-rate.csv", function(data) {
        const DATA = data.map(function(o) {
            return {
                "Town" : o["Town"],
                "Year" : dateFormat.parse(o["Year"]),
                "Eligibility Rate" : parseFloat(o["Value"]) / 100
            };
        });
        ...
        function drawChart(container_width) {
            ...
            Object.keys(classNames).forEach(function(town) {
                ...
                var lineFunction = d3.svg.line()
                    ...
                    .y(function(d) { console.log(y(d["Eligibility Rate"])); return y(d["Eligibility Rate"]); })
            ...
            hoverGroups.each(function(pointData, i) {
                ...
                var points = group.selectAll("path.point")
                    ...
                    .append("path")
                        ...
                        .attr("transform", function(d) { return "translate(" + x(d["Year"]) + ", " + y(d["Eligibility Rate"]) +")";})

                pointData.map(function(town) {
                    // make text labels
                    var valueText = [
                        town["Town"],
                        decimalPercentFormat(town["Eligibility Rate"])
                    ];
