        ...
        function drawChart(container_width) {
            ...
            DATA.map(function(d) {
                Object.keys(classNames).forEach(function(indicator) {
                    yDomain[d[indicator]] = true;
                })
            })

            yDomain = d3.extent(Object.keys(yDomain).map(function(k) { return parseInt(k); }));
            console.log(yDomain);
            ...
            hoverGroups.each(function(pointData, i) {
                ...
                Object.keys(classNames).forEach(function(indicator) {
                    if (!isNaN(pointData[indicator])) {
                        ...
                        var valueText = [
                            indicator,
                            (isNaN(pointData[indicator]) ? "NA" : numberFormat(pointData[indicator]))
                        ];
